...
class Target:
    ...
    # initally in pp_functions.utils
    ...
    def update(self, pp): 
        ...
        if self.passed == False and np.linalg.norm(self.position - pp.car.position) <= 20/pp.ppu: 
            self.passed = True
            self.visible = False
        ...
        if np.linalg.norm(self.position - pp.car.position) < pp.car.fov / pp.ppu:
            ...
            if np.abs(self.alpha) < pp.car.fov_range and self.passed == False:
                self.visible = True
            else:
                self.visible = False
        else:
