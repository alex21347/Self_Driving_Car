...
def render(pp):
   ...
   # if path_midpoints != 0 and len(path_midpoints) > 0:
    ...
    for key in pp.cone.new_visible_cone_flag.keys():
        for i in range(len(pp.cone.boundary_sample_draw[key][0])):
            text_surf = pygame.font.Font(None, 8).render(f'x', 1, (255, 255, 255))
            text_pos = [(pp.cone.boundary_sample_draw[key][0][i] * pp.ppu) + offset[0], (pp.cone.boundary_sample_draw[key][1][i] * pp.ppu) + offset[1]]
            pp.screen.blit(text_surf, text_pos)       
